...
class ZapPlugin(ScannerPlugin):
    ...
    def run(self, context: ScanContext) -> tuple[ScannerRun, list[Finding], bool] | None:
        ...
        try:
            ...
            scan_id = self._start_spider(api_url, target_url, apikey)
            self._wait_spider(api_url, scan_id, timeout, apikey)
            self._wait_passive(api_url, timeout, apikey)
            alerts = self._collect_alerts(api_url, target_url, apikey)
    ...
    def _start_spider(self, api_url: str, target_url: str, apikey: str | None) -> str:
    ...
    def _wait_spider(self, api_url: str, scan_id: str, timeout_seconds: int, apikey: str | None) -> None:
    ...
    def _wait_passive(self, api_url: str, timeout_seconds: int, apikey: str | None) -> None:
        deadline = time.monotonic() + timeout_seconds
        while time.monotonic() < deadline:
            payload = _api_get_json(
    ...
    def _collect_alerts(self, api_url: str, target_url: str, apikey: str | None) -> list[dict[str, Any]]:
        start_at = 0
        count = 500
...
def _api_get_json(url: str, params: dict[str, str], apikey: str | None) -> dict[str, Any]:
    payload = dict(params)
    if apikey:
        payload["apikey"] = apikey
    query = urllib.parse.urlencode(payload)
    request_url = f"{url}?{query}" if query else url
    with urllib.request.urlopen(request_url, timeout=20) as response:  # noqa: S310
